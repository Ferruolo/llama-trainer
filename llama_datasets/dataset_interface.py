...
dataset_tools = {
    "grammar": (
       ...
       (process_grammar_dataset, get_grammar_idx, load_grammar_data, get_grammar_len)
    )
}
...
class dataloader(Dataset):
    ...
    def __getitem__(self, index):
        ...
        (input_, target_) = self.getter(dataset, index)

        prompt_ids = self.tokenizer.encode(input_, add_special_tokens=False)
        label_ids = self.tokenizer.encode(output_, add_special_tokens=False)
